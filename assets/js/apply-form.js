...
const STSApplyForm = (function () {
  ...
  // Telefon alanı için ülke kodu + beklenen hane sayısı listesi.
  // "digits: 0" olanlar (Diğer) için hane sayısı doğrulanmaz.
  const COUNTRY_PHONE_CODES = [
    { code: '90', name: 'Türkiye', flag: '🇹🇷', digits: 10 },
    { code: '7', name: 'Rusya', flag: '🇷🇺', digits: 10 },
    { code: '380', name: 'Ukrayna', flag: '🇺🇦', digits: 9 },
    { code: '994', name: 'Azerbaycan', flag: '🇦🇿', digits: 9 },
    { code: '995', name: 'Gürcistan', flag: '🇬🇪', digits: 9 },
    { code: '998', name: 'Özbekistan', flag: '🇺🇿', digits: 9 },
    { code: '993', name: 'Türkmenistan', flag: '🇹🇲', digits: 8 },
    { code: '996', name: 'Kırgızistan', flag: '🇰🇬', digits: 9 },
    { code: '992', name: 'Tacikistan', flag: '🇹🇯', digits: 9 },
    { code: '', name: 'Diğer', flag: '🌐', digits: 0 }
  ];
  ...
  function render(container, formDef, options) {
    ...
    formDef.fields.forEach(function (f) {
      ...
      if (isTwoSided) {
        ...
        html += '<div class="apply-phone-group">' +
          '<select class="apply-input apply-phone-country" id="field_' + f.FieldID + '_country">' +
            COUNTRY_PHONE_CODES.map(c => '<option value="' + c.code + '" data-digits="' + c.digits + '">' + c.flag + ' +' + (c.code || '?') + '</option>').join('') +
          '</select>' +
          '<input type="tel" class="apply-input apply-phone-number" id="field_' + f.FieldID + '" placeholder="5xx xxx xx xx" inputmode="numeric">' +
        '</div>' +
        '<div class="apply-phone-hint" id="phonehint_' + f.FieldID + '"></div>';
    ...
    // Telefon alanları — ülke koduna göre canlı hane doğrulaması
    container.querySelectorAll('.apply-phone-number').forEach(function (numberInput) {
      const fieldId = numberInput.id.replace('field_', '');
      const countrySelect = container.querySelector('#field_' + fieldId + '_country');
      const hintEl = container.querySelector('#phonehint_' + fieldId);

      function checkPhone() {
        const digits = numberInput.value.replace(/\D/g, '');
        const opt = countrySelect.options[countrySelect.selectedIndex];
        const expectedDigits = parseInt(opt.getAttribute('data-digits'), 10) || 0;
        if (expectedDigits > 0 && digits.length > 0 && digits.length !== expectedDigits) {
          hintEl.textContent = expectedDigits + ' haneli olmalı (şu an ' + digits.length + ' hane girdiniz)';
      ...
      numberInput.addEventListener('input', checkPhone);
      countrySelect.addEventListener('change', checkPhone);
    });
    ...
    function collect() {
      ...
      formDef.fields.forEach(function (f) {
        ...
        if (f.Type === 'Telefon') {
          const countryEl = container.querySelector('#field_' + f.FieldID + '_country');
          const numberEl = container.querySelector('#field_' + f.FieldID);
          const code = countryEl ? countryEl.value : '';
          const digits = numberEl ? numberEl.value.replace(/\D/g, '') : '';
          formData[f.FieldID] = digits ? ('+' + code + ' ' + digits) : '';
          return;
        }
    ...
    function validate() {
      for (let i = 0; i < formDef.fields.length; i++) {
        ...
        if (f.Type === 'Telefon') {
          ...
          const numberEl = container.querySelector('#field_' + f.FieldID);
          const digits = numberEl ? numberEl.value.replace(/\D/g, '') : '';
          if (isRequired && options.mode !== 'edit' && !digits) {
          ...
          if (digits && countryEl) {
            ...
            if (expectedDigits > 0 && digits.length !== expectedDigits) {
              return f.Label + ' için geçerli bir numara giriniz (' + expectedDigits + ' hane bekleniyor).';
            }
          }
          continue;
        }
